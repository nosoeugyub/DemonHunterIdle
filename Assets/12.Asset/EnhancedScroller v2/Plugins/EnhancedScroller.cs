...
{
    /// <summary>
    /// 셀 뷰의 가시성 변경을 처리하는 델리게이트
    /// </summary>
    /// <param name="cellView">가시성이 변경된 셀 뷰</param>
    public delegate void CellViewVisibilityChangedDelegate(EnhancedScrollerCellView cellView);

    /// <summary>
    /// 셀 뷰가 재활용되기 직전에 호출되는 델리게이트
    /// </summary>
    /// <param name="cellView"></param>
    public delegate void CellViewWillRecycleDelegate(EnhancedScrollerCellView cellView);

    /// <summary>
    /// ScrollRect의 스크롤 콜백을 처리하는 델리게이트
    /// </summary>
    /// <param name="scroller">델리게이트를 호출한 스크롤러</param>
    /// <param name="val">스크롤 Rect의 스크롤 값</param>
    /// <param name="scrollPosition">스크롤러 시작 지점부터의 픽셀 단위 스크롤 위치</param>
    public delegate void ScrollerScrolledDelegate(EnhancedScroller scroller, Vector2 val, float scrollPosition);

    /// <summary>
    /// 스크롤러의 스냅핑을 처리하는 델리게이트
    /// </summary>
    /// <param name="scroller">델리게이트를 호출한 스크롤러</param>
    /// <param name="cellIndex">스냅된 셀 뷰의 인덱스 (looping인 경우 데이터 인덱스와 다를 수 있음)</param>
    /// <param name="dataIndex">스냅된 데이터의 인덱스</param>
    ...
    /// <summary>
    /// 스크롤러 상태 변경(스크롤 중 또는 스크롤 중이 아님)을 처리하는 델리게이트
    /// </summary>/// <param name="scroller">상태가 변경된 스크롤러</param>
    /// <param name="scrolling">스크롤러가 스크롤 중인지 여부</param>
    public delegate void ScrollerScrollingChangedDelegate(EnhancedScroller scroller, bool scrolling);

    /// <summary>
    /// 스크롤러 상태 변경(점프 중 또는 점프 중이 아님)을 처리하는 델리게이트
    /// </summary>
    /// <param name="scroller">상태가 변경된 스크롤러</param>
    /// <param name="tweening">스크롤러가 트윈 중인지 여부</param>
    public delegate void ScrollerTweeningChangedDelegate(EnhancedScroller scroller, bool tweening);

    /// <summary>
    ///  셀 뷰가 처음 생성될 때 호출되는 델리게이트 (재사용되지 않음)
    /// </summary>
    /// <param name="scroller">셀 뷰를 생성한 스크롤러</param>
    /// <param name="cellView">생성된 셀 뷰</param>
    public delegate void CellViewInstantiated(EnhancedScroller scroller, EnhancedScrollerCellView cellView);
    
    /// <summary>
    /// 재활용된 셀 뷰가 재사용될 때 호출되는 델리게이트
    /// </summary>
    /// <param name="scroller">셀 뷰를 재사용한 스크롤러</param>
    /// <param name="cellView">재사용된 셀 뷰</param>
    public delegate void CellViewReused(EnhancedScroller scroller, EnhancedScrollerCellView cellView);

    /// <summary>
    /// EnhancedScroller는 동적 스크롤러를 쉽게 설정할 수 있도록 하며, 뷰를 자동으로 재활용합니다. 
    /// 이는 소수의 뷰만으로도 수천 개의 행을 표시할 수 있음을 의미합니다. 
    /// 이를 통해 메모리와 처리 능력을 절약할 수 있습니다.
    /// </summary>
    ...
    public class EnhancedScroller : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        /// <summary>
        /// 셀의 어느 쪽을 참조할지 결정합니다.
        /// 세로 스크롤러의 경우, before는 위쪽, after는 아래쪽을 의미합니다.
        /// 가로 스크롤러의 경우, before는 왼쪽, after는 오른쪽을 의미합니다.
        /// </summary>
        ...
        /// <summary>
        /// 데이터에 따라 스크롤 바를 어떻게 표시할지 설정합니다. 
        /// 스크롤 바가 연결되어 있지 않으면 무시됩니다. 
        /// OnlyIfNeeded는 스크롤러가 루핑되거나 스크롤할 항목이 충분하지 않은 경우 스크롤 바를 숨깁니다.
        /// </summary>
        ...
        /// <summary>
        /// 첫 번째 셀 뷰 이후 셀 뷰 사이의 픽셀 수입니다.
        /// </summary>
        public float spacing;

        /// <summary>
        /// 스크롤러 내부의 패딩: 상단, 하단, 왼쪽, 오른쪽.
        /// </summary>
        public RectOffset padding;

        /// <summary>
        /// 스크롤러가 셀 뷰를 루핑할지 여부입니다.
        /// </summary>
        [SerializeField]
        private bool loop;

        /// <summary>
        /// 드래그 중 루프 점프를 처리할지 여부입니다.
        /// 참고: 이 값이 꺼져 있을 때 작은 목록 크기를 사용할 경우, 
        /// 목록 가장자리 근처에서 드래그할 때 요소가 누락될 수 있습니다. 
        /// 이 값을 끄면 목록 끝 근처에서 드래그할 때 Unity가 많은 속도를 추가하는 것을 방지할 수 있습니다. 
        /// 이 값이 꺼져 있으면 maxVelocity 값을 0이 아닌 값으로 설정하여 큰 관성 속도를 완화할 수 있습니다(예: maxVelocity 참조).
        /// </summary>
        public bool loopWhileDragging = true;

        /// <summary>
        /// 스크롤러가 이동할 수 있는 최대 속도입니다. 
        /// 이를 통해 사용자의 과도한 스크롤링을 방지할 수 있습니다. 
        /// 또한 목록 끝 근처에서 드래그하고 루핑할 때 Unity가 추가하는 큰 관성 속도를 완화하는 데 사용할 수 있습니다(예: loopWhileDragging 참조).
        /// </summary>
        ...
        /// <summary>
        /// 스크롤러가 이 속도로 감속할 때 스냅이 시작됩니다. 
        /// 스크롤러가 이 속도로 감속할 때 지정된 위치로 스냅됩니다.
        /// </summary>
        public float snapVelocityThreshold;

        /// <summary>
        /// 스냅 오프셋입니다. 스냅이 발생할 때, 스냅할 셀을 결정하는 위치입니다.
        /// 일반적으로 오프셋은 0..1 범위에 있으며, 0은 스크롤러의 상단/왼쪽, 1은 하단/오른쪽입니다.
        /// 대부분의 상황에서 watch offset과 jump offset은 동일하며, 추가 기능이 필요할 경우에만 분리됩니다.
        /// </summary>
        /// <summary>
        public float snapWatchOffset;

        /// <summary>
        /// 셀이 이동할 스냅 위치입니다. 스냅이 발생할 때, 스냅된 셀이 이동할 위치입니다.
        /// 일반적으로 오프셋은 0..1 범위에 있으며, 0은 셀의 상단/왼쪽, 1은 하단/오른쪽입니다.
        /// 대부분의 상황에서 watch offset과 jump offset은 동일하며, 추가 기능이 필요할 경우에만 분리됩니다.
        /// </summary>
        public float snapJumpToOffset;

        /// <summary>
        /// 셀이 스크롤러 위치에 스냅된 후, 이 값은 셀이 그 스크롤러 위치에 중앙에 맞춰지는 방식을 결정합니다.
        /// 일반적으로 오프셋은 0..1 범위에 있으며, 0은 셀의 상단/왼쪽, 1은 하단/오른쪽입니다.
        /// </summary>
        public float snapCellCenterOffset;

        /// <summary>
        /// 셀 간 간격을 고려하여 셀 오프셋을 정렬할지 여부입니다.
        /// </summary>
        public bool snapUseCellSpacing;

        /// <summary>
        /// 현재 스크롤 위치와 스냅 위치 사이를 보간하는 데 사용할 함수입니다. 이는 이징(easing)이라고도 합니다.
        /// 스냅 위치로 즉시 이동하려면 snapTweenType을 immediate로 설정하거나 snapTweenTime을 0으로 설정할 수 있습니다.
        public TweenType snapTweenType;

        /// <summary>
        /// 현재 스크롤 위치와 스냅 위치 사이를 보간하는 데 걸리는 시간입니다.
        /// 스냅 위치로 즉시 이동하려면 snapTweenType을 immediate로 설정하거나 snapTweenTime을 0으로 설정할 수 있습니다.
        /// </summary>
        public float snapTweenTime;

        /// <summary>
        /// true인 경우 스크롤러가 드래그되는 동안 계속해서 스냅합니다.
        /// false인 경우 드래그가 멈출 때까지 스냅을 비활성화합니다.
        /// </summary>
        public bool snapWhileDragging;

        /// <summary>
        /// 스크롤러가 드래그를 멈출 때 (스냅이 true인 경우) 스냅이 발생하도록 합니다.
        /// 터치가 스크롤러를 움직였지만 이후 정지한 경우 유용합니다.
        /// </summary>
        public bool forceSnapOnEndDrag;

        /// <summary>
        /// 터치가 스크롤러에 재참여하면 스냅 트윈을 중지합니다.
        /// </summary>
        public bool interruptTweeningOnDrag;

        /// <summary>
        /// true인 경우 트윈이 처리되지 않습니다. false인 경우 트윈이 재개됩니다.
        /// </summary>
        public bool tweenPaused;

        /// <summary>
        /// 스크롤러 위치 앞쪽에 미리 볼 공간의 양입니다. 
        /// 처음 보이는 셀보다 앞에 있어도 로드될 수 있습니다. 
        /// 트윈 및 외부 리소스 로드에 유용합니다.
        /// </summary>
        ...
        /// <summary>
        /// 스크롤러 위치 뒤쪽에 미리 볼 공간의 양입니다. 
        /// 마지막 보이는 셀 뒤에 있어도 로드될 수 있습니다. 
        /// 트윈 및 외부 리소스 로드에 유용합니다.
        /// </summary>
        ...
        public float ScrollPosition
        {
            ...
            set
            {
				if (loop)
				{
                    // 루프 모드인 경우, 새 위치가 점프 트리거를 초과하지 않도록 조정합니다.
                    // 초과할 경우, 영역의 반대쪽 점프 위치로 초기화해야 합니다.
                ...
                if (_scrollPosition != value)
                {
                    ...
                    // 스크롤 방향에 따라 ScrollRect의 정규화된 위치를 설정합니다.
        ...
        /// <summary>
        /// 스크롤러의 가시적인 부분을 제외한 활성 셀 뷰 컨테이너의 크기
        /// </summary>
        ...
        /// <summary>
        /// 스크롤러가 결과적인 셀 뷰를 루프할지 여부를 결정합니다.
        /// 루프는 스크롤러가 세 개의 내부 크기 데이터 세트를 생성하며,
        /// 스크롤러를 중간 세트에 유지하려고 시도합니다. 스크롤러가 이 세트를 벗어나면,
        /// 중간 세트로 다시 점프하여 데이터의 무한한 세트처럼 보이게 합니다.
        /// </summary>
        public bool Loop
        {
            get
            ...
            set
            {
                ...
                if (loop != value)
                {
                    // 루프를 켜기 전에 원래 위치를 기록하여
                    // 루프를 켤 때 이 위치로 다시 점프할 수 있습니다.
                    var originalScrollPosition = _scrollPosition;

                    loop = value;

                    // 루프가 켜져 있으면 더 많은 내부 요소를 생성하고,
                    // 루프가 꺼져 있으면 요소를 제거합니다.
                    _Resize(false);

                    if (loop)
                    {
                        // 데이터의 중간 세트+원래 위치를 기준으로 새 스크롤 위치를 설정합니다
                        ScrollPosition = _loopFirstScrollPosition + originalScrollPosition;
                    }
                    else
                    {
                        // 원래 위치와 첫 번째 루프 위치를 기반으로 새로운 스크롤 위치를 설정합니다.
        ...
        public ScrollbarVisibilityEnum ScrollbarVisibility
        {
            ...
            set
            {
                ...
                // 스크롤바가 존재하는 경우에만 실행합니다.
                if (_scrollbar != null)
                {
                    // 셀 뷰가 실제로 존재하는지 확인합니다.
                    if (_cellViewOffsetArray != null && _cellViewOffsetArray.Count > 0)
                    {
                        ...
                        {
                            // 스크롤 가능한 영역의 크기가 스크롤러보다 작거나
                            // 루프가 켜져 있는 경우, 가시성이 항상으로 설정되어 있지 않은 한
                            // 스크롤바를 숨깁니다.
                            _scrollbar.gameObject.SetActive(scrollbarVisibility == ScrollbarVisibilityEnum.Always);
                        }
                        else
                        {
                            // 스크롤 가능한 영역의 크기가 스크롤러보다 크거나
                            // 루프가 꺼져 있는 경우, 가시성이 절대로로 설정되어 있지 않은 한
                            // 스크롤바를 표시합니다.
        ...
        /// <param name="keepPosition">현재 포지션을 유지할것인지 여부(원래 에셋에서 확장 제작함)</param>
        public void ReloadData(float scrollPositionFactor = 0, bool keepPosition = false)
        {
            _reloadData = false;
