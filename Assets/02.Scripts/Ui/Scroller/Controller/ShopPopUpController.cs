...
/// 클래스용도 : 상점 탭 컨틀롤러
/// </summary>
public class ShopPopUpController : MonoBehaviour, IEnhancedScrollerDelegate
{
    ...
    void Start()
    {
        ...
        SetData(GameDataTable.Instance.ShopResoucrceGolddataDic);
    }
    ...
    public void AddNewRow()
    {
        ...
        int newKey = _data.Count > 0 ? _data.Keys.Max() + 1 : 0;
        _data.Add(newKey, new ShopGoldCellData());
    ...
    // 스크롤러를 조정하는 함수
    public void ResizeScroller(bool keepPosition = false)
    {
    ...
    {
        var cellView = scroller.GetCellView(cellViewPrefab) as ShopCellView;
        cellView.SetData(_data[dataIndex +1 ]); //해당데이터는 인덱스가 1부터시작함
