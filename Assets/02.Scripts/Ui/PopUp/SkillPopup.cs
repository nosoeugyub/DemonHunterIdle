...
public class SkillPopup : MonoBehaviour, IPopUp
{
    ...
    //SkillSlot objs
    public RectTransform[] CategorySubArray; //하단팝업
    public GameObject HorizontalSkillSlotPrefab;  // Prefab for horizontal slot container
    public GameObject SkillSlotPrefab;  // Prefab for individual skill slot
    ...
    private void ActiveHunterPortraits() //Contrain에 값에따라서 탭 매뉴 활성화
    {
        ...
        for (int i = 0; i < Upgradeinbtnarray.Length; i++)
        {
            ...
            {
                Upgradeinbtnarray[i].gameObject.SetActive(false);
            }
        }
    }
    ...
    { //클릭한 인덱스 열기
        ...
        if ( i <= 2 && GameDataTable.Instance.User.HunterPurchase[i] == false)//3은 공용스킬이라..무시
        {
