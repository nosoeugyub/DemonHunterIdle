...
/// 클래스용도 : 가디언스킬레이아웃
/// /// </summary>
public class GuardianSKillLayout : MonoBehaviour
{
    ...
    public void Init_Slots(List<ISkill> currentequippedSkills)
    {
        ...
        //데이터 분배
        SplitHunterSkillSlotIndex(GameDataTable.Instance.User.GuardianSkillSlotindex);
        //강화도 적용
        ...
        {
            PresetButton[i].SetTypeButton(Utill_Enum.ButtonType.DeActive);
        }
        ...
        for (int i = 0; i < HunterSKillarray.Length; i++)
        {
            ...
            if (row < 4 && col < 4) // 배열 인덱스 범위 체크
            {
                ...
                HunterSKillarray[i].Rowindex = row+1; // RowIndex 설정 스킬인덱스는 1부터시작
            }
            else
            {
        ...
        //프리셋 0번으로 일단 설정
        CurrentPreset = GameDataTable.Instance.User.Euipmentuserpreset[1];
        string[] preset = GetPreset(CurrentPreset);
        //battleUi에도 이미지 업데이트를 해줘야함 
        Update_BattleUiView(Utill_Enum.SubClass.Guardian, preset);
        //장착된 스킬이있다면 스킬사용
        int j = 1;
        for (int i = 0; i < 4; i++)
        {
    ...
    public void presetClick(int i)
    {
        ...
        switch (i)
        {
            case 0: //1번프리셋
                temp = preset1;
                isbuy = User.isCheckBuySkillPreset(GameDataTable.Instance.User, Utill_Enum.SubClass.Guardian, i);
                break;
            case 1: //2번프리셋
                temp = preset2;
                isbuy = User.isCheckBuySkillPreset(GameDataTable.Instance.User, Utill_Enum.SubClass.Guardian, i);
        ...
        if (isbuy) //구매가되있으면
        {
            ...
            //프리셋도 변경
            CurrentPreset = i;
            User.Set_Presetinedex(GameDataTable.Instance.User, CurrentPreset, Utill_Enum.SubClass.Guardian);
            //battleUi에도 이미지 업데이트를 해줘야함 
            Update_BattleUiView(Utill_Enum.SubClass.Guardian, GetPreset(CurrentPreset));
            //프리셋데이터로 ui바꿔줘야함
            UpdateView(temp, i);
        }
        else
    ...
    private void ClickSlot(ISkill skill, SkillUiElement slot)
    {
        ...
        else
        {
            ...
            {
                if (BattleSkillUis[i].Skill != null && BattleSkillUis[i].Skill.SkillName == skill.SkillName)
                {
                    BattleSkillUis[i].Empty_Slot();//아예 빈 슬롯으로
                    BattleSkillUis[i].Skill = null;//스킬데이터 해제
                }
            }
            return;
        }
    ...
    public void AllSkillUse(string[] skillname)
    {
    ...
    {
        if (!DataManager.Instance.Hunters[1].gameObject.activeSelf) //헌터가없으면 스킬발동안함
        {
            //발동안할때는 비활성화
            for (int i = 0; i < BattleSkillUis.Length; i++)
            {
        ...
        if (GameStateMachine.Instance.CurrentState is NonCombatState) //현재 비전투 상태라면
        {
            BattleSkillUis[index].SetCanRunningSkill(false);
            BattleSkillUis[index].StartCooldown();
            return;
        }
        BattleSkillUis[index].isUnuse = true;
        BattleSkillUis[index].InitSlot(skill.SkillCoolDown , skill , Utill_Enum.SubClass.Guardian, isEquip);
        BattleSkillUis[index].StartCooldown();
    }
